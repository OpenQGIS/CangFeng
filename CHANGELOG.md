...
- **底部悬浮工具栏底板不透明度调优至 60% 轻透毛玻璃 (Floating Toolbar 60% Frosted-Glass Base)**：
  - 将底部悬浮工具栏底板背景不透明度精确调整至 `60%`（深色 `rgba(14, 18, 24, 0.60)` / 浅色 `rgba(255, 255, 255, 0.60)`）；
  - 搭配 `16px` 高质感毛玻璃滤镜与细腻柔和阴影，达成“轻微透明、不完全遮挡底层地图细节”的极佳视觉体验，同时确保所有操作按键与文字清晰锐利。
