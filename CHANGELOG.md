...
- **底部悬浮工具栏重塑为双轨制自适应液态玻璃 (Adaptive Optical Liquid Glass Architecture)**：
  ...
  - **环境反射与棱镜色散**：胶囊外圈叠加 `0 0 0 1px / 2px` 红蓝双轨棱镜微色散环，并内覆 `mix-blend-mode: overlay` 环境色彩吸收光膜，让边缘反光根据底层地图实时融入（海洋蓝、沙漠金、雪山白），彻底告别死白机械线条；
  - **通透革命**：基底底色透光率大幅提升至 65%，配合 `backdrop-filter: blur(12px) saturate(210%) brightness(1.10)` 高阶透镜增透，使底图山川纹理自然鲜亮穿透；
  - **桌面端专属**：激活多层微棱镜宝石切面折射光弧与鼠标悬停流光呼吸感，呈现 VisionOS 水晶体拟态；
  - **移动端专属**：去除琐碎内阴影脏边，引入单条高反差 `1.5px` 水滴反射顶弧与触控按下（`:active`）果冻弹性微挤压反馈，并轻量化模糊半径，确保移动端巨幅地图缩放 120fps/60fps 满帧如丝。
