...
- **底部悬浮工具栏底板不透明度精调至 95% (Floating Toolbar 95% Solid Base)**：
  - 响应视觉沉稳需求，将深色/浅色模式下的底部悬浮工具栏底座背景不透明度统一调整至 `95%`（`rgba(14, 18, 24, 0.95)` / `rgba(255, 255, 255, 0.95)`）；
  - 配合 `20px` 高度磨砂滤镜与轻量边框，既保留极微弱的底图色彩渗透感，又具备更强的实体沉稳度，彻底防止底图繁杂细节干扰底栏操作按键。
