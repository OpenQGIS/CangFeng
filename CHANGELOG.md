...
- **二维码分享弹窗手机横屏与矮屏自适应适配 (QR Code Modal Landscape & Mobile Responsive Optimization)**：
  - 根除手机横屏（高度 320px~375px）下因弹窗高度 438px 导致头部关闭按钮与底部操作按钮被两头截断死锁的严重体验缺陷；
  - 引入 `max-height: min(92vh, 92dvh)` 与内容区 `overflow-y: auto` 弹性滚动兜底双保险；
  - 在 `@media (max-height: 520px)` 下自动切换紧凑模式（二维码微缩至 124px，内边距按比压缩，总高收缩至 267px），无需滑动即可在横屏下一屏全览；
  - 移动端竖屏（`max-width: 600px`）优化底部操作按钮为 1:1 双列等宽通栏，消除副标题硬编码并完备四国语言（中/英/日/韩）国际化字典。
